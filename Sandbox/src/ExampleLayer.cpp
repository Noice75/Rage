...
{
	m_VertexArray = Rage::VertexArray::Create();
	...
	Rage::Ref<Rage::VertexBuffer> vertexBuffer = Rage::VertexBuffer::Create(vertices, sizeof(vertices));
	Rage::BufferLayout layout = {
		{ Rage::ShaderDataType::Float3, "a_Position" },
		{ Rage::ShaderDataType::Float4, "a_Color" }
	};
	vertexBuffer->SetLayout(layout);
	m_VertexArray->AddVertexBuffer(vertexBuffer);

	uint32_t indices[3] = { 0, 1, 2 };
	Rage::Ref<Rage::IndexBuffer> indexBuffer = Rage::IndexBuffer::Create(indices, sizeof(indices) / sizeof(uint32_t));
	...
	Rage::Ref<Rage::VertexBuffer> squareVB = Rage::VertexBuffer::Create(squareVertices, sizeof(squareVertices));
	squareVB->SetLayout({
		{ Rage::ShaderDataType::Float3, "a_Position" },
	...
	Rage::Ref<Rage::IndexBuffer> squareIB = Rage::IndexBuffer::Create(squareIndices, sizeof(squareIndices) / sizeof(uint32_t));
	...
	m_ChernoLogoTexture = Rage::Texture2D::Create("assets/textures/ChernoLogo.png");
...
void ExampleLayer::OnUpdate(Rage::Timestep ts) 
{
	// Update
	...
	for (int y = 0; y < 20; y++)
	{
		for (int x = 0; x < 20; x++)
		{
			...
			Rage::Renderer::Submit(m_FlatColorShader, m_SquareVA, transform);
		}
	}
...
void ExampleLayer::OnEvent(Rage::Event& e) 
{
